...
> 分支：`refactor/project-audit`（14 个提交 = 11 个代码提交 + 3 个文档提交）
...
上表 **11 个代码提交**，另有 3 个只改 `AUDIT.md` 的文档提交，共 14 个。
代码侧改动 **149 个文件**、**+11817 / -10440 行**，其中约 95% 是纯格式（`--fix`）。
每个提交前都重跑三条门禁，无中间破态。

复现这两个数字（`HEAD~1` 是为了排除本报告的文档提交）：

```bash
git log --oneline main..HEAD          # 提交清单
git diff --shortstat main..HEAD~1     # 代码侧改动量
```

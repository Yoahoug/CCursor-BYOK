...
import type { CursorDynamicToolDefinition } from './dynamicTools'
import type { McpRoutingEntry, McpStateServerInfo } from './mcpState'
import type { ParsedRunRequest } from './protocol/types'
import type { AgentSession } from './session'
import type { ToolResultEnvelope } from './toolResults'
import type { AvailableDynamicBuiltinTool, AvailableMcpTool, ToolCallInfo } from './tools'
...
import { fetchMcpState, mergeMcpStateIntoRoutingTable } from './mcpState'
import { getBackgroundJob, registerBackgroundJob } from './session'
...
import { buildEditPlan, buildExecArgs, mapToolToExecArgs, resolveToolCall } from './tools'
...
async function* runToolCallInner(params: Parameters<typeof runToolCall>[0]): AsyncGenerator<AgentServerMessage, void, void> {
  ...
  if (cursorToolType === 'getMcpToolsToolCall') {
    ...
    if (servers === null && namespaces.length === 0) {
      logger.warn({ callId: tc.callId, queryMode }, '[DYNAMIC-TOOLS] discovery failed — client returned no MCP state')
      rawToolResult = {
        result: {
          case: 'error',
      ...
      if (servers === null) {
        logger.warn({ callId: tc.callId, queryMode }, '[DYNAMIC-TOOLS] MCP discovery failed — returning available cursor namespace')
      }

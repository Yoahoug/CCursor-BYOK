...
仪表盘数字对不上账、浮层透字、以及悬停提示在淡出时闪一下。

### 模型列表支持拖动排序

顺序是有意义的：Cursor 的模型选择器按 `providers.json` 里 `models` 数组的顺序
渲染（见 `server/config/providersStore.ts` 的 `flattenModels`），常用模型排在前面
能少滚动。所以这里改的是**真实数组顺序**，会和其它字段一样进 draft 走
dirty → 保存流程，而不是只在界面上排序。

几处刻意的选择：

- **独立握把，而不是整条头部可拖** —— 头部已绑了点击展开，两者叠在一起时想展开
  手一抖就变成排序。图标用纯 CSS 画（codicon 里没有 gripper 一类字形）。
- **插入位置按卡片中线判定**，不看鼠标移动方向 —— 后者在"往上拖但还没越过中线"时
  会让插入点提前跳变，看着比实际更靠前。
- **拖动全程不改数组，drop 时一次做完**，否则列表在鼠标底下重排，反而更难放到位。
- **插入线用伪元素画**而不是 border：改 border 会让卡片高度变化，鼠标一移动列表就抖。
- 被拖走的卡片留半透明而不是 `display: none`，否则下方卡片立刻补位，用户会丢失
  "我从哪里拖出来"的参照。

重排逻辑抽到 `src/shared/listReorder.ts` 并补测试（9 例）。这里有个容易漏的分支：
`splice` 摘掉被拖元素后，目标元素下标会整体左移一位，沿用摘除前的下标会让**向下拖**
插错位置 —— 向上拖恰好正确，所以手测很容易放过。

### 仪表盘按「今日 / 全部累计」拆分

原先只有一个合计，语义是「最近 14 天窗口」，但界面上没写清，会被读成「累计总量」；
真去对账才发现对不上，而且窗口一滚动，同一个数字还会自己变小。

改为一趟遍历同时算出三个口径，各自显式标注范围：`today`（本地时区当天）、
`allTime`（全部记录，不受窗口限制）、`window`（最近 14 天，趋势图与「按模型」表用）。
今日在上、累计在下而不是并排 —— 两块回答的是不同问题（"今天有没有白花" vs
"总体上省了多少"），时间尺度也不同，并排会诱使人横向比较。

顺带修掉两处读数问题：

- 「全部」副标题原本写 `since <起点>`，单端会被当成当前日期（数据只攒了两天时，
  `since 09-16` 看着就像面板没刷新）。改为给完整的 `起点 → 终点` 区间，右端取真实
  的最晚记录时间而不是"今天"，免得几天没用时看着像新数据。
- 「暂无数据」原先判的是窗口内 calls，超过 14 天没用过的用户会看到空面板，而累计
  区块其实有数。改用 `allTime` 判定。

### 修复浮层透字

弹窗、图表悬浮读数面板、下拉/提示三处浮层都用「当前前景色叠一小层」的 surface
当底（4~11% 墨水）。这个做法在卡片背景上没问题，但浮在**已有内容**之上时会直接
透出底下的字：弹窗盖在表单上，底下字段名透上来和弹窗文字叠成两层；读数面板浮在
柱子和指标卡上，数字叠数字。

- **弹窗**改用不透明的控件底色（`editorWidget.background` 本来就是给这类浮层的
  颜色，两套主题都成立）；阴影改用黑色而不是 `--cpp-ink`（前景色在深色主题下接近
  白，做阴影会变成一圈发白光晕）；遮罩换成纯黑压暗，去掉 `backdrop-filter`。
- **悬浮件**补齐取值链：`editorHoverWidget.*` / `editorWidget.*` 这几个颜色 VS Code
  **是有值**的，但值来自色彩注册表的默认项（`editorHoverWidget.background` 默认 =
  `editorWidget.background`），主题 JSON 里查不到 —— 直接写
  `var(--vscode-editorHoverWidget-background)` 会一路落回兜底，于是浮层变半透明。

### 修复图表悬停提示闪出空盒子

鼠标移出柱状图后，提示框会先闪一下小方块再消失。原因是「关闭」与「清空」被当成了
同一件事：`clearUsageHover` 立刻把状态置空，而淡出动画有 120ms —— 这段过渡里
`x-text` 的日期和 `x-for` 的数值行都取不到数据，盒子遂收缩到 `min-width` + 内边距，
变成一个空框闪一下；同时 `left` 落回兜底的 `50%`，所以它还会先横跳到图表中央。

拆开这两个动作：新增 `usageTipLast` 记住最后一次悬浮的目标，`clearUsageHover`
只关不删，取值一律走 `usageHover ?? usageTipLast`，让提示框带着文字整体淡下去。

### 上游模型列表显示可读名，并修正解析

「Fetch models」拿到的列表原先只显示 id。部分中转站会把 id 混淆成不可读的串
（把 `flash` 倒写成 `hsalf` 之类），整列看下来没有一行认得出。

改为两项布局：主行用上游给的 `display_name`，副行是真实 id（等宽字体，方便逐字
比对），两者一致时不显示副行。配套修正解析逻辑 —— 之前有三个形状没覆盖到：只读
camelCase 的 `displayName`（该中转站返回 snake_case，于是可读名被整个丢掉）、
时间戳只按秒级处理（毫秒与 ISO 字符串会排错序）、只认顶层数组（`{ data: [...] }`
与 `{ models: [...] }` 都解不出来）。

解析逻辑抽到 `src/shared/remoteModels.ts` 并补测试（15 例）。应用某个上游模型时，
也一并把 `display_name` 填进 Display Name（这正是最容易手敲错的地方），只在用户
没填过时才写，不覆盖已改好的名字。

### 开发：面板前端本地预览

面板 UI 的验证原先只能靠装一遍扩展再看，改一行样式要走完整的构建 → 覆盖扩展目录
→ 重启流程，配色和间距这类需要反复试的参数基本没法调。

`npm run dev:ui` 起一个本地预览：复用**真实的** panel-provider 与 webview 代码，
只替换接入层（`vscode` 模块、`acquireVsCodeApi`、以及会抢端口的 `src/server`），
其余配置 store、用量统计、模型测试都是真代码。预览脚本与产物不进 VSIX。
